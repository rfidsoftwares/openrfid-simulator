# OpenRFID Simulator Desktop App User Guide
...
**OpenRFID Simulator Desktop App** is a native Windows desktop application designed to simulate physical RFID hardware. It allows developers to build, test, and demonstrate RFID-enabled software applications (such as inventory, logistics, or asset-tracking systems) without requiring physical RFID readers and tag assets.
...
## 3. Installation & Getting Started (Desktop Version)

To install and run the standalone desktop application on Windows:

1. **Get the Installer**:
   Locate the generated installer file:
   * **MSI Format**: `OpenRFID Simulator_1.0.0_x64_en-US.msi`
   * **EXE Format**: `OpenRFID Simulator_1.0.0_x64-setup.exe`
2. **Run the Installer**:
   Double-click the installer and accept the EULA license agreement (RFID Software India Private Limited).
3. **Run the Application**:
   Launch the application from the Start Menu or your Desktop shortcut. The UI will open immediately, and the Hopeland UDP/TCP background engine will run automatically in the background (as a secure binary sidecar) without requiring any terminal windows or console setups.
