        ...
        <div key={date} style={{ ...card, marginBottom: '10px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <span style={{ fontSize: '18px', cursor: 'pointer' }} onClick={() => handleExpandDate(date)}>
                {expandedDate === date ? '▼' : '▶'}
              </span>
              <div>
                <span style={{ fontWeight: 'bold', fontSize: '15px', cursor: 'pointer' }} onClick={() => handleExpandDate(date)}>{date}</span>
